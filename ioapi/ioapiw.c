...
#ifdef _WIN32
#include <io.h>
#include "iowin32.c" /* sadly we need WIN32FILE_IOWIN */
#else
#include <wctype.h>
...
local voidpf ZCALLBACK fopen_file_funcW(voidpf opaque, const wchar_t* filename, int mode)
{
   FILE* file = NULL;
#ifdef _WIN32
...
/*
static BOOL win32_chsize(HANDLE file, ZPOS_T size)
{
}
*/
   ...
   int fopen_setlength(voidpf opaque, voidpf stream, ZPOS_T size)
   {
      ...
      if (0 == fgetpos(file, &oldpos))
      {
         if (0 == fsetpos(file, &newpos))
         {
            ...
            /*
            if (win32_chsize(HANDLE file, ZPOS_T size))
            {
            }
            */
            if (1 && SetEndOfFile(handle))
            {
            ...
            {
            /*
               typedef BOOL (WINAPI* FN)(HANDLE, LONGLONG);
               FN fn = (FN)GetProcAddress(LoadLibrary("kernel32.dll"), "SetFileValidData");
               if (fn && (*fn)(handle, size))
               {
                  ret = fsetpos(file, &oldpos);
               }
               else
...
int zgets_file(void* stream, const struct zlib_filefunc_def_s* api, char* szDst, int nMax, int* bUnicode)
{
   char* sz = szDst;
   wchar_t* wsz = NULL;
   BOOL bContinue;
   ...
   for (bContinue = TRUE; bContinue && (count < (nMax - 1)); )
   {
      if (*bUnicode)
      {
         ...
         bContinue = ZREAD(*api, stream, wsz + count, sizeof(*wsz));
         if (bContinue) switch (wsz[count])
         {
            ...
            case '\r':
               count++;
               bContinue = ZREAD(*api, stream, wsz + count, sizeof(*wsz));
               if (bContinue && wsz[count] != '\n')
               {
                  ZSEEK(*api, stream, -(int)sizeof(*wsz), ZLIB_FILEFUNC_SEEK_CUR);
               } // else eat it
               bContinue = FALSE;
               break;
            case '\0':
      ...
      {
         bContinue = ZREAD(*api, stream, sz + count, sizeof(*sz));
         ...
         if (bContinue) switch (sz[count])
         {
            ...
            case '\r':
               count++;
               bContinue = ZREAD(*api, stream, sz + count, sizeof(*sz));
               if (bContinue && sz[count] != '\n')
               {
                  ZSEEK(*api, stream, -(int)sizeof(*sz), ZLIB_FILEFUNC_SEEK_CUR);
               } // else eat it
               bContinue = FALSE;
               break;
...
int zgets_fileW(void* stream, const struct zlib_filefunc_defW_s* api, wchar_t* szDst, int nMax, int* bUnicode)
{
   wchar_t* wsz = NULL;
